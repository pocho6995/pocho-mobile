...
class CarServiceDetailPage extends StatefulWidget {
  const CarServiceDetailPage({super.key, required this.carService});

  final CarService carService;

  static const String routeName = '/car-service-detail';

  @override
  State<CarServiceDetailPage> createState() => _CarServiceDetailPageState();
}

class _CarServiceDetailPageState extends State<CarServiceDetailPage> {
  ...
  // Мок фотографии СТО
  final List<String> _carServiceImages = [];

  // Рекламные блоки
  final List<Map<String, dynamic>> _ads = [
    {
      'title': 'Скидка 15% на услуги',
      'subtitle': 'При обслуживании от 500 000 сум',
    ...
    },
    {
      ...
      'subtitle': 'При ремонте от 200 000 сум',
      'icon': Icons.construction_rounded,
      'color': [0xFF059669, 0xFF10B981],
      'bgGradient': [0xFF059669, 0xFF10B981],
    },
  ];
  ...
  void initState() {
    ...
    if (carService.imageUrl.isNotEmpty) {
      _carServiceImages.add(carService.imageUrl);
    }
    // Добавляем мок фотографии
    _carServiceImages.addAll([
  ...
  Future<void> _toggleFavorite() async {
    ...
    try {
      ...
      if (success && mounted) {
        setState(() {
          carService.isFavorite = !carService.isFavorite;
        });
      } else if (mounted) {
  ...
  void _startImageAutoScroll() {
    if (_carServiceImages.length > 1) {
      Future.delayed(const Duration(seconds: 5), () {
        if (mounted && _imagePageController.hasClients) {
          final nextPage = (_currentImageIndex + 1) % _carServiceImages.length;
  ...
  void _openInMaps() async {
    final url = Uri.parse(
      'https://www.google.com/maps/search/?api=1&query=${carService.latitude},${carService.longitude}',
    );
  ...
  void _callCarService() async {
    final url = Uri.parse(
      carService.phone != null
          ? 'tel:${carService.phone}'
          : 'tel:+998901234567',
    );
    if (await canLaunchUrl(url)) {
      await launchUrl(url);
  ...
  Widget build(BuildContext context) {
    final position = LatLng(carService.latitude, carService.longitude);
        ...
        slivers: [
          ...
          SliverAppBar(
            ...
            flexibleSpace: FlexibleSpaceBar(
              background: _carServiceImages.isEmpty
                  ? Container(
                      decoration: const BoxDecoration(
                  ...
                  : Stack(
                      ...
                      children: [
                        PageView.builder(
                          ...
                          itemCount: _carServiceImages.length,
                          onPageChanged: (index) {
                            setState(() {
          ...
          SliverToBoxAdapter(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: isSmallScreen ? 12 : 16),
                ...carService.features.map((feature) {
                  return Padding(
                    padding: EdgeInsets.symmetric(
                      horizontal: isSmallScreen ? 16 : 20,
                    ),
                    child:
                        Container(
                            ...
                            .fadeIn(
                              ...
                              delay:
                                  (700 +
                                          carService.features.indexOf(feature) *
                                              100)
                                      .ms,
                            )
                            .slideX(
                ...
                if (carService.features.isEmpty) ...[
                  Container(
                    margin: EdgeInsets.symmetric(
                      horizontal: isSmallScreen ? 16 : 20,
                    ),
                ...
                SizedBox(height: isSmallScreen ? 20 : 24),
                // Описание
                if (carService.description.isNotEmpty) ...[
                  Padding(
                        padding: EdgeInsets.symmetric(
                          horizontal: isSmallScreen ? 16 : 20,
                        ),
                      ...
                      child: Column(
                        children: [
                          ...
                          _ModernInfoRow(
                            ...
                            label: 'Координаты',
                            value:
                                '${carService.latitude.toStringAsFixed(6)}, ${carService.longitude.toStringAsFixed(6)}',
                            gradient: const [
                              Color(0xFF10B981),
                              Color(0xFF059669),
                            ],
                          ...
                          _ModernInfoRow(
                            icon: Icons.phone_rounded,
                            label: 'Телефон',
                            value: carService.phone ?? '+998 90 123 45 67',
                            onTap: _callCarService,
                            gradient: const [
                              Color(0xFF059669),
                              Color(0xFF10B981),
...
// Виджеты для детальной страницы СТО
