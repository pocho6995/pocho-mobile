...
class ChargingStationDetailPage extends StatefulWidget {
  const ChargingStationDetailPage({super.key, required this.station});

  final ChargingStation station;

  static const String routeName = '/charging-station-detail';

  @override
  State<ChargingStationDetailPage> createState() =>
...
class _ChargingStationDetailPageState extends State<ChargingStationDetailPage> {
  ...
  // Мок фотографии электрозаправки
  final List<String> _stationImages = [];

  // Рекламные блоки
  final List<Map<String, dynamic>> _ads = [
    {
      'title': 'Скидка 15% на зарядку',
      'subtitle': 'При зарядке от 50 кВт·ч',
      'icon': Icons.ev_station_rounded,
      'color': [0xFF8B5CF6, 0xFFA78BFA],
      'bgGradient': [0xFF8B5CF6, 0xFFA78BFA],
    },
    {
      'title': 'Бесплатная парковка',
      'subtitle': 'При зарядке на станции',
      'icon': Icons.local_parking_rounded,
      'color': [0xFFA78BFA, 0xFFC4B5FD],
      'bgGradient': [0xFFA78BFA, 0xFFC4B5FD],
    },
  ];
  ...
  void initState() {
    ...
    if (station.imageUrl.isNotEmpty) {
      _stationImages.add(station.imageUrl);
    }
  ...
  void _callStation() async {
    final url = Uri.parse(
      station.phone != null ? 'tel:${station.phone}' : 'tel:+998901234567',
    );
    if (await canLaunchUrl(url)) {
      await launchUrl(url);
    }
                    ...
                    child:
                        Container(
                            ...
                            .fadeIn(
                              ...
                              delay:
                                  (700 +
                                          station.features.indexOf(feature) *
                                              100)
                                      .ms,
                            )
                            .slideX(
                      ...
                      child: Column(
                        children: [
                          ...
                          _ModernInfoRow(
                            icon: Icons.phone_rounded,
                            label: 'Телефон',
                            value: station.phone ?? '+998 90 123 45 67',
                            onTap: _callStation,
                            gradient: const [
                              Color(0xFFC4B5FD),
                              Color(0xFFA78BFA),
...
// Виджеты для детальной страницы электрозаправки
