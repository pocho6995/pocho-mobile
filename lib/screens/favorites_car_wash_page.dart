...
class FavoritesCarWashPage extends StatefulWidget {
  const FavoritesCarWashPage({super.key});

  static const String routeName = '/favorites-car-wash';

  @override
  State<FavoritesCarWashPage> createState() => _FavoritesCarWashPageState();
}

class _FavoritesCarWashPageState extends State<FavoritesCarWashPage> {
  ...
  final List<Map<String, dynamic>> _adBanners = [
    {
      ...
      'subtitle': 'Получайте уведомления о лучших автомойках первыми',
  ...
  Future<void> _loadFavorites() async {
    ...
    try {
      ...
      final response = await _carWashService.getFavoriteCarWashes(
        skip: 0,
        limit: 100,
      );

      setState(() {
        _favorites = response.carWashes;
        _isLoading = false;
      });
    } catch (e) {
  ...
  // Категории для навигации (без текущей категории "Мойки")
              ...
              sliver: SliverList(
                delegate: SliverChildBuilderDelegate(
                  (context, index) {
                    ...
                    // Карточка автомойки
                    if (carWashIndex >= _favorites.length) {
                      return const SizedBox.shrink();
                    }
                    final carWash = _favorites[carWashIndex];
                    return Padding(
                      padding: const EdgeInsets.only(bottom: 16),
                      child:
                          _FavoriteCarWashCard(
                                carWash: carWash,
                                index: carWashIndex,
                                onFavoriteChanged: () async {
                                  // Удаляем из избранного через API
                                  try {
                                    final success = await _carWashService
                                        .removeFromFavorites(carWash.id);
                                    if (success && mounted) {
                                      setState(() {
                                        _favorites.remove(carWash);
                                      });
                                    }
                                  } catch (e) {
                                    // Игнорируем ошибки
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Добавляйте автомойки в избранное,\nчтобы быстро находить их',
            style: TextStyle(
              fontSize: 16,
...
class _FavoriteCarWashCard extends StatelessWidget {
  const _FavoriteCarWashCard({
